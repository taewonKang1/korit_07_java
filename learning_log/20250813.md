...
```
이상의 형태가 python에서는 dictionary라는 표현을 쓰고, java에서는 또 map이 라고 씁니다.
person1.name = "김이"
...
### 재사용성
- 제네릭을 사용하면 다양한 자료형에 대해 하나의 클래스나 메서드를 정의 할 수 있어 코드의 재사용성을 높이고 중복을 줄입니다,

    - 예를 들어 method overloading 개념이 있을 경우, Person 객체를 참조하여 bmi를 계산한다고 가정해보겠습니다.
...
* 참고 : 사실은 upcasting이 이루어지기 때문에 별 문제가 없습니다.
...
## 제네릭
- 클래스, 인터페이스, 메서드에 사용되는 자료형을 매개변수화 할 수 있도록 하는것으로 다양한 타입에 재사용 가능
...
value에 String / Integer / Double / Product / Item ... etc 등으로 사용가능합니다. 즉 Java 자체의 자료형(class) 뿐만 아니라 개발자가 정의한 것도 집어넣을 수 있다는 점에서 재사용성이 높다고 할 수 있습니다.
