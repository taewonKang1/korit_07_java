...
/*
    ...
    예를 들어 Computer / AirConditional / Speaker들이 전부 Power를
    ...
    A의 자식 클래스가 B 하나 뿐일떄만 적용된다고 볼 수 있습니다.
    A의 자식 클래스가 B C D E F ... 여러 개가 있다고 가정한다면
    그것을 업캐스팅했을 때 A가 가지고 있는 부모 메서드를 호출함으로서
    다수의 서로 다른 클래스의 객체들을 통제할 수 있다는 장점 때문에
    업캐스팅을 적용하게 됩니다.
    그리고 다운캐스팅을 해서 각각 클래스의 고유 메서드도 실행시킬 수 이싿.

    AirConditioner 클래스 생성해서 Power implement하고
    에어컨의 전원을 켭니다
    에어컨의 전원을 끕니다로 재정의하세요
    Speaker도 동일하게 하시오.
