...
public class CentralControl {
    ...
    public void addDevice(Power device) {
        ...
        // 비어있는 element에 device 추락를 할 수 있어야 합니다.
        // 그렇다면 필요한 기능이 배열 내에 null 값이 존재하는지에 대한 체크가 필요하다고
        // 할 수 있습니다.
        int emptyIndex = checkEmpty();  // method를 결과값의 변수 emptyIndex에 대입
        if(emptyIndex == -1) {
        ...
        /*
            ...
            .getClass().getSimppleName() -> 클래스명만 출력됨.
    ...
    private int checkEmpty() { // 메서드인데 private 썼습니다. Main에서 굳이
        ...
        for(int i = 0; i < deviceArray.length; i++ ){
            if(deviceArray[i] == null) {
                return i;               // 즉 배열 내부에는 element가 없다면 그 주소지를 return
            }
        }
        return -1;
        /*
            ...
            음수값을 지정하는 경우도 있는데,
            나중에 위에 있는 addDecivice() 메서드에서 if(checkEmpty() == -892345)로 쓰고 앂지 않으면
            -1을 쓰는게 가장 보완적입니다
         */
    }
    ...
    /*
        ...
        deviceArray를 반복 돌려서 몇 번 인덱스에 어떤 클래스이 객체가 있는지 표시하는
        showInfo 메서드를 정의하시오.
        실행 예
        슬롯 [ 1 ] 번 : Computer
        슬롯 [ 2 ] 번 : LED
