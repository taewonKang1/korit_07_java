...
public class Main {
    public static void main(String[] args) {
        ...
        dog1.makeSound();
        dog1.fetch();
        dog1.makeSound();       // 결과값 : 개가 짖습니다. 즉 재정의된 method가 호출됨
...
//        animal2.fetch();         // 불가능
        ...
        /*
            ...
            그럼 어차피 재정의된 method 버전으로 호출이 될꺼라면 애초에
        ...
        System.out.println("↑ 업캐스팅 파트 - 객체명.메서드명(); 했을 때 재정의 버전이 나오는가 아닌가 주목");
        System.out.println("ↆ 다운캐스팅 파트");
        /*
...
//        dog3.makeSound(); // 그러니까 얘는 지금 호출도 못해보는 상황입니다.
        /*
            74번 라인에서 오류가 발생헀습니다.
            즉 애초에 animal3를 생성할 때 Animal 클래스이 생성자를 호출해서 만들었기 때문에
            (A a = new A();이기 때문에)
            하위 클래스이 인스턴스로 다운캐스팅을하는 것이 불가능합니다.

            이상을 해결하기 위해서
         */
        Animal animal4 = new Dog();
        // 형태로 선언된 클래스와 호출되는 생성자가 서로 달라야 합니다.(그리고 둘은 상속관계여야 합니다)
