...
/*
    ...
    heal() -> 체력 10을 회복하고, 현재 체력 출력
        체력이 10 회복되었습니다. 현재 체력 : health(숫자로 출력되어야 합니다)
...
class Character {
    ...
    public void attack() {
        ...
        // 공격하고 나서 경험치를 얻고 싶습니다. -> 글너데 외부에서 공격 이외의 방식으로 경험치를 강제저긍로
        // 획득하는 버그는 막고 싶어요. 그렇다면 위의 가이드에서 작성했던 getExp()는 외부에서 실행되면 안됩니다.
        this.gainExp(30);
    }
...
//        warrior.setName("전사");          // 이상의 직접 대입 방식과의 코드 작성 차이점에 대해서 확인 바람
...
/*
    이상의 코드에서 여러분들이 중점저긍로 학습해야 할 점은
    접근 지정자의 설정 수준에 따라서 setter / getter가 필요한 사례가 있고 아닌 사례가 있다는 점입니다.
    예를 들어 계업을 하거나 SNS를 한다고 쳤을 때 닉네임의 경우는 누구나 볼 수 있죠.
    그렇다면 닉네임 필드는 public으로 설정되어있을겁니다.
    그렇기 때문에 getter를 통한 닉네임의 호출은 필요가 없어서 이상의 코드에서 getName()메서드는 쓸모가
    없을겁니다.

    우리는 어떤 클래스/필드/메서들르 어떤 접근지정자를 설정해야할지도 고민해야겠습니다.

    하지만 현재 수업 상황에서는 웬만하면 필드는 private로 설정, getter / setter를 통한 우회 설정 및 조회를
    목표로 하겠습니다.
 */
