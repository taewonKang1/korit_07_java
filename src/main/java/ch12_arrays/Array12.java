...
/*
    배열의 출력
    엄밀히 말하자면 저희는 배열을 출력한게 아니라 여태까지 배열 내부의 elemet들을 출력했습니다.
    ...
    main에 반복문 작성하든지 method를 정의하다느지 하는 일은 번거로운 일이 됩니다.
...
public class Array12 {
    public static void main(String[] args) {
        ...
        // int[] numbers배열에 1, 2, 3, 4, 5 ... 10 까지 대입해 두세요.
        ...
        // 오름 차순 정렬
        Arrays.sort(nums);      // 예의 결과값이 void니까 원본 배열을 바꿈
        ...
        /*
            이상의 경우는 보면 sort() 메서드에 오름차순에서는 argument가 하나 였는데,
            내림 차순할 떄는 보니까 argument가 두 개 입니다
            이상의 과정이 overloading이 이루어졌다는 점을 파악할 수 있으면 현재 수준으로는
            만족스럽겠습니다.
            그리고 두 번째 argument로 Comparartor.reverseOrder(); 가 사용되었는데
            Comparator.reverseOrder()의 결과값/return값이 sort() 메서드의
            두 번째 argument로 사용되었다는 점에서 역시 함수형 프로그램의 일종이라고
            볼 수 있겠네요
