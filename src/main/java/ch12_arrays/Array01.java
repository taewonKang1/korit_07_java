...
//        System.out.println(arr01);          // 결과값 : 1
        ...
        // 어, index만 빼고 나머지가 반복되네? 반복문 써봐야겠네? 하면 개발자 지망생
        // 반복문을 활용하여 arr01의 내부 element들을 콘솔에 출력하시오.
        // 1 2 3 4 5 로 출력될 수 있도록 수정하시오.
        for (int i = 0 ; i < 5 ; i++ ) {
            System.out.println(arr01[i] + " ");     // i가 어디에 적용되어야하는지 감안할 필요가 있었다
        }

        for (int i = 0; i < arr01.length; i++) {    // arr01.lenth 형태로 배열 내부의 데이터 개수가 바뀌더라도 문제 없음
            System.out.println(arr01[i] + " / ");
        }

        "안녕하세요".length();

        // 근데 우리가 필드를 참조할 떄 객체명.필드 형태로 참조했었습니다.
        ...
        // arr02에 21, 22, 23 ... 30 을 대입하고
        // 21 / 22  이라고 출력될 수 있도록 코드를 작성하시오
        for (int i = 0; i < arr02.length; i++) {
            arr02[i] = i + 21;
