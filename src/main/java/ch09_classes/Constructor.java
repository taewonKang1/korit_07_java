...
/*
    ...
    생성자의 정의 방식
        1. 기본 생성자 : 매개변수가 없는 생성자
            (사실 클래스를 만들 때 default로 생성되는 형태로 정으하지 않아도 될 때가 있음)
            그래서 ClassA, Car에서 기본 생성자를 정의하지 않고
            ClassAMain, CarMain에서 객체를 생성해봤었습니다.
                정확하게는 default로 생성된 기본생성자를 Main에서 호출해봤다고 볼 수 있습니다.
                호출 결과 -> 객체 생성
...
public class Constructor {
    ...
    // 그래고 매개변수의 유무만 있어서 call1() / call2() 유형이라고 했으니까
    // 기본 생성자는 call1() 유형에 해당할 것이고, 클래스명과 동일한 메서드명을 가집니다.
    Constructor() {
        System.out.println("NoArgsConstructor(기본생성자)");
    }

    // 매개변수 생성자 정의 -> 기본저긍로 만들어지지 않습니다. 개발자가 정의해야 함.
    ...
    // 작성한 뒤
    // ConstructorMain으로 가서 , constructor3라는 객체명을 지니고, title에 "여러분 이름"으로
    // 지어 객체를 생성한 후,
